...
const Splash = () => {
  ...
  }, ocaBundleResolver, historyEnabled] = useServices([TOKENS.CACHE_SCHEMAS, TOKENS.CACHE_CRED_DEFS, TOKENS.SCREEN_TERMS, TOKENS.UTIL_LOGGER, TOKENS.UTIL_LEDGERS, TOKENS.CONFIG, TOKENS.UTIL_OCA_RESOLVER, TOKENS.HISTORY_ENABLED]);
    ...
    const initOnboarding = async () => {
      try {
        ...
        if (store.onboarding.onboardingVersion !== OnboardingVersion) {
          dispatch({
            type: DispatchAction.ONBOARDING_VERSION,
        ...
        const error = new BifoldError(t('Error.Title1044'), t('Error.Message1044'), (err === null || err === void 0 ? void 0 : err.message) ?? err, 1044);
      ...
      try {
        ...
        if (agent) {
          ...
          try {
            ...
            throw new BifoldError('Wallet Service', 'There was a problem unlocking the wallet.', error.message, 1047);
          }
