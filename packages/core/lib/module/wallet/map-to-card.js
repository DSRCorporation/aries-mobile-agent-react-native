...
import { isPredicate } from './ui-types';
...
const mapItemToCardAttr = (item, labels, formats, flaggedPII) => {
  ...
  const isPII = !isPredicate(item) ? !!(flaggedPII !== null && flaggedPII !== void 0 && flaggedPII.includes(rawLabel)) : false;
  return {
    key,
    label,
    value,
...
export const brandingOverlayTypeString = type => {
  switch (type) {
...
export function mapAnonCredsToCard(rec, bundle, opts = {}) {
  const {
    proofContext = false,
    revoked = false,
    notInWallet = false,
...
export function mapW3CToCard(input, id, opts = {}) {
  ...
  const items = proofContext && displayItems !== null && displayItems !== void 0 && displayItems.length ? displayItems.map(it => mapItemToCardAttr(it, input.labels, input.formats, input.piiKeys)) : Object.entries(subject).map(([key, raw]) => {
    ...
    const label = ((_input$labels = input.labels) === null || _input$labels === void 0 ? void 0 : _input$labels[key]) ?? startCase(key);
...
const mapW3CCredToCard = (w3cCred, brandingOverlay, brandingOverlayTypeString, opts = {}) => {
  ...
  const extraAttributeValue = credentialDisplay.display.primary_overlay_attribute ? (_getAttributeField = getAttributeField(credentialDisplay, credentialDisplay.display.primary_overlay_attribute)) === null || _getAttributeField === void 0 ? void 0 : _getAttributeField.field : undefined;
...
export async function mapCredentialTypeToCard({
  ...
  //W3C case
  if (credential instanceof W3cCredentialRecord || credential instanceof W3cV2CredentialRecord || credential instanceof SdJwtVcRecord || credential instanceof MdocRecord) {
  ...
  const isRevoked = !!(credentialErrors !== null && credentialErrors !== void 0 && credentialErrors.includes(CredentialErrors.Revoked));
  const notInWallet = !!(credentialErrors !== null && credentialErrors !== void 0 && credentialErrors.includes(CredentialErrors.NotInWallet));
  const params = {
