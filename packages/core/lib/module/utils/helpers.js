...
import { DidCommCredentialState, DidCommProofState, DidCommOutOfBandRole, DidCommBasicMessageRole } from '@credo-ts/didcomm';
 ...
 */
export const getAttributeFormats = bundle => {
...
const addMissingDisplayAttributes = async (attrReq, records, agent) => {
  ...
  for (const attributeName of [...(names ?? (name && [name]) ?? [])]) {
    ...
    if (attribute && attribute.length > 0) {
      ...
      (_processedAttributes$2 = processedAttributes.attributes) === null || _processedAttributes$2 === void 0 || _processedAttributes$2.push(new Attribute({
        revoked: false,
        credentialId: credName,
        name: attributeName,
        value: '',
...
export const processProofAttributes = async (t, request, credentials, credentialRecords, groupByReferent, agent) => {
  const processedAttributes = {};
  ...
  for (const key of Object.keys(retrievedCredentialAttributes)) {
    ...
    for (const credential of credentialList) {
      ...
      for (const attributeName of [...(names ?? (name && [name]) ?? [])]) {
        ...
        (_processedAttributes$4 = processedAttributes[credential.credentialId].attributes) === null || _processedAttributes$4 === void 0 || _processedAttributes$4.push(new Attribute({
          ...requestedProofAttributes[key],
          revoked,
