...
exports.formatDcqlCredentialsForRequest = formatDcqlCredentialsForRequest;
exports.formatDifPexCredentialsForRequest = formatDifPexCredentialsForRequest;
exports.formatOpenIdProofRequest = formatOpenIdProofRequest;
...
const getDcqlClaimFormat = record => {
  switch (record.type) {
    ...
    case 'SdJwtVcRecord':
      return _core.ClaimFormat.SdJwtDc;
    default:
...
const getDcqlDisclosedPayload = validCredential => {
  ...
  if (validCredential.record.type === 'SdJwtVcRecord') {
    return (0, _display.filterAndMapSdJwtKeys)(asRecord(output)).visibleProperties;
  }
...
function formatDcqlCredentialsForRequest(queryResult) {
      ...
      return {
        ...
        credentials: validCredentials.map(validCredential => {
          const {
            display,
            metadata
          } = (0, _display.getCredentialForDisplay)(validCredential.record);
...
function formatOpenIdProofRequest(record) {
