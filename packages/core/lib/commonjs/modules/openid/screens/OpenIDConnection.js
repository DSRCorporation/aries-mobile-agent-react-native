...
var _openid = require("../hooks/openid");
...
const OpenIDConnection = ({
  ...
  const [logger, historyEnabled] = (0, _containerApi.useServices)([_containerApi.TOKENS.UTIL_LOGGER, _containerApi.TOKENS.HISTORY_ENABLED]);
  const {
    ColorPalette
  } = (0, _theme.useTheme)();
  const notificationRecord = (0, _openid.useOpenID)({
    openIDUri,
    openIDPresentationUri
  });
  ...
  (0, _react.useEffect)(() => {
    ...
    if ((0, _credentialRecord.isOpenIDCredentialRecord)(notificationRecord)) {
    ...
    if ((0, _credentialRecord.isOpenIdProofRequestRecord)(notificationRecord)) {
