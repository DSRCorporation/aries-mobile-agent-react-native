...
function getCredentialForDisplay(credentialRecord) {
  ...
  if (credentialRecord instanceof _core.SdJwtVcRecord) {
    ...
    const {
      disclosures,
      jwt
    } = (0, _decode.decodeSdJwtSync)(credentialRecord.firstCredential.compact, (data, alg) => _core.Hasher.hash(data, alg));
    ...
    return {
      ...
      claimFormat: _core.ClaimFormat.SdJwtW3cVc,
  ...
  if (credentialRecord instanceof _core.MdocRecord) {
    ...
    const mdocInstance = _core.Mdoc.fromBase64Url(credentialRecord.firstCredential.base64Url);
  ...
  const credential = _core.JsonTransformer.toJSON(credentialRecord.firstCredential.claimFormat === _core.ClaimFormat.JwtVc ? credentialRecord.firstCredential.credential : credentialRecord.firstCredential);
  const openId4VcMetadata = (0, _metadata.getOpenId4VcCredentialMetadata)(credentialRecord);
  ...
  return {
    ...
    metadata: {
      ...
      issuedAt: (0, _utils.formatDate)(new Date(credential.issuanceDate)),
      validUntil: credential.expiryDate ? (0, _utils.formatDate)(new Date(credential.expiryDate)) : undefined,
      validFrom: undefined
    },
