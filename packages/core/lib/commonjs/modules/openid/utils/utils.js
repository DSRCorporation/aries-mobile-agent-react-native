...
exports.buildFieldsFromOpenIDTemplate = void 0;
exports.detectImageMimeType = detectImageMimeType;
exports.formatDate = formatDate;
exports.getCredentialConfigurationIds = getCredentialConfigurationIds;
exports.getHostNameFromUrl = getHostNameFromUrl;
exports.isDateString = isDateString;
exports.isSdJwtProofRequest = isSdJwtProofRequest;
exports.isW3CProofRequest = isW3CProofRequest;
...
function getHostNameFromUrl(url) {
  try {
    return (0, _core.getDomainFromUrl)(url);
  } catch (error) {
    throw new Error(`Error getting hostname from url: ${error}`);
  }
}
const buildFieldsFromOpenIDTemplate = data => {
  const fields = [];
...
exports.buildFieldsFromOpenIDTemplate = buildFieldsFromOpenIDTemplate;
...
function isW3CProofRequest(type) {
  if (type === 'jwt_vc') {
    return true;
  }
  return false;
}
function isSdJwtProofRequest(type) {
  if (type === 'vc+sd-jwt') {
    return true;
  }
  return false;
}
function getCredentialConfigurationIds(resolved) {
  const fromOffered = (resolved.credentialOfferPayload.credential_configuration_ids ?? []).filter(x => !!x);
