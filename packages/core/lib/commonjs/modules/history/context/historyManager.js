...
class HistoryManager {
  logger = new _logger.BifoldLogger();
  constructor(agent) {
    this.agent = agent;
    if (!agent) {
  ...
  async saveHistory(recordData) {
    ...
    try {
      const historySettingsOption = await _storage.PersistentStorage.fetchValueForKey(_constants.LocalStorageKeys.HistorySettingsOption);
  ...
  async handleStoreHistorySettings(selectedValue) {
    ...
    await _storage.PersistentStorage.storeValueForKey(_constants.LocalStorageKeys.HistorySettingsOption, selectedValue.id);
  ...
  async getStoredHistorySettingsOption() {
    return (await _storage.PersistentStorage.fetchValueForKey(_constants.LocalStorageKeys.HistorySettingsOption)) ?? null;
  }
