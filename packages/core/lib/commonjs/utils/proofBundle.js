...
const useRemoteProofBundleResolver = (indexFileBaseUrl, log) => {
  ...
  const [resolver, setResolver] = (0, _react.useState)(new DefaultProofBundleResolver(proofRequestTemplates));
  (0, _react.useEffect)(() => {
    if (indexFileBaseUrl) {
...
class RemoteProofBundleResolver extends _fileCache.FileCache {
  cacheDataFileName = _constants.templateCacheDataFileName;
  constructor(indexFileBaseUrl, log) {
    super(indexFileBaseUrl, _constants.templateBundleStorageDirectory, _constants.templateCacheDataFileName, log);
  }
  ...
  async checkForUpdates() {
    ...
    await this.loadBundleIndex(_constants.templateBundleIndexFileName);
  }
...
class DefaultProofBundleResolver {
  constructor(proofRequestTemplates) {
    this.proofRequestTemplates = proofRequestTemplates ?? _verifier.getProofRequestTemplates;
  }
  async resolve(acceptDevRestrictions) {
