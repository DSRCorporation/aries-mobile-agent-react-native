...
class FileCache {
  ...
  saveFileToLocalStorage = async (fileName, data) => {
    ...
    try {
      var _this$log2;
      await (0, _reactNativeFs.writeFile)(pathToFile, data, 'utf8');
  ...
  fileStoragePath = () => {
    return `${_reactNativeFs.CachesDirectoryPath}/${this.workspace}`;
  };
  createWorkingDirectoryIfNotExists = async () => {
    const path = this.fileStoragePath();
    const pathDoesExist = await (0, _reactNativeFs.exists)(path);
    if (!pathDoesExist) {
      try {
        await (0, _reactNativeFs.mkdir)(path);
        return true;
      } catch {
        var _this$log4;
  ...
  loadFileFromLocalStorage = async fileName => {
    ...
    try {
      ...
      const data = await (0, _reactNativeFs.readFile)(pathToFile, 'utf8');
  ...
  checkFileExists = async fileName => {
    ...
    try {
      var _this$log8;
      const fileExists = await (0, _reactNativeFs.exists)(pathToFile);
